...
## inspect
Inspects a single NEO (Near Earth Object), printing its details in a human-readable format.
...
## query
Query subcommand generates a collection of close approaches that match a set of specified files, and either displays a limited set of those results to standard
output or writes the structured results to a file.
...
Interactive subcommand loads the database and then starts a command loop so that you can repeatedly run
`inspect` and `query` subcommands on the database without having to wait to reload the data each time you want
